...
acquire_lock() {
    ...
    if ! flock -n 9; then
        ...
        log "⚠️  ATTENTION : le script est déjà en cours d'exécution (PID ${pid}). Abandon." >&2
        exit 1
    fi
    echo $$ >&9
}
...
# --- LOGGING ---
HOSTNAME=$(hostname -s)
log() { echo "$(date '+%Y-%m-%dT%H:%M:%S') ${HOSTNAME} : $*"; }
# ---------------
...
log "=== Tile Cache Purge Started ==="

FREE_SPACE=$(get_free_space)
log "Current free space: $((FREE_SPACE / 1024 / 1024)) GB (${FREE_SPACE} KB)"
log "System load (1min): ${CURRENT_LOAD} (threshold: ${MAX_LOAD})"

if [ "${IS_LOW_LOAD}" = "1" ]; then
    ACTIVE_THRESHOLD=${MIN_FREE_KB}
    log "Mode: normal (low load) — purge threshold: $((ACTIVE_THRESHOLD / 1024 / 1024)) GB"
else
    ACTIVE_THRESHOLD=${EMERGENCY_FREE_KB}
    log "Mode: emergency-only (high load) — purge threshold: $((ACTIVE_THRESHOLD / 1024 / 1024)) GB"
fi

# Sortie rapide si l'espace est suffisant
if [ "${FREE_SPACE}" -ge "${ACTIVE_THRESHOLD}" ]; then
    log "Sufficient free space available. Nothing to do."
    exit 0
fi

log "Low disk space. Starting purge (target: $((TARGET_FREE_KB / 1024 / 1024)) GB free, safety floor: ${MIN_AGE} days)..."
...
while IFS= read -r -d '' entry; do
    ...
    if [ $((count % BATCH_SIZE)) -eq 0 ]; then
        FREE_SPACE=$(get_free_space)
        log "  [${count} files deleted] Free space: $((FREE_SPACE / 1024 / 1024)) GB"
...
if $target_reached || [ "${FREE_SPACE}" -ge "${TARGET_FREE_KB}" ]; then
    log "Success: ${count} files deleted. Free space: $((FREE_SPACE / 1024 / 1024)) GB"
elif [ "${FREE_SPACE}" -ge "${ACTIVE_THRESHOLD}" ]; then
    log "Partial: ${count} files deleted. Free space: $((FREE_SPACE / 1024 / 1024)) GB (above trigger threshold, below target)"
else
    log "WARNING: ${count} files deleted but free space ($((FREE_SPACE / 1024 / 1024)) GB) still below trigger threshold. No tiles older than ${MIN_AGE} days remain."
fi

log "=== Tile Cache Purge Finished ==="
